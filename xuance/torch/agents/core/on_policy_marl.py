...
class OnPolicyMARLAgents(MARLAgents):
    ...
    def __init__(
        ...
        self.memory: Optional[Union[MARL_OnPolicyBuffer, MARL_OnPolicyBuffer_RNN]] = (
            None
        )
    ...
    def _build_memory(self):
        ...
        if self.use_actions_mask:
            avail_actions_shape = {
                key: (self.action_space[key].n,) for key in self.agent_keys
            }
        else:
    ...
    def store_experience(
        ...
        **kwargs,
        ...
        experience_data = {
            "obs": {
                k: np.array([data[k] for data in obs_dict]) for k in self.agent_keys
            },
            "actions": {
                k: np.array([data[k] for data in actions_dict]) for k in self.agent_keys
            },
            "log_pi_old": log_pi_a,
            "rewards": {
                k: np.array([data[k] for data in rewards_dict]) for k in self.agent_keys
            },
            "values": values_dict,
            "terminals": {
                k: np.array([data[k] for data in terminals_dict])
                for k in self.agent_keys
            },
            "agent_mask": {
                k: np.array([data["agent_mask"][k] for data in info])
                for k in self.agent_keys
            },
        }
        if self.use_rnn:
            experience_data["episode_steps"] = np.array(
                [data["episode_step"] - 1 for data in info]
            )
        if self.use_global_state:
            experience_data["state"] = np.array(kwargs["state"])
        if self.use_actions_mask:
            experience_data["avail_actions"] = {
                k: np.array([data[k] for data in avail_actions])
                for k in self.agent_keys
            }
    ...
    def init_rnn_hidden(self, n_envs):
        ...
        if self.use_rnn:
            ...
            rnn_hidden_actor = {
                k: self.policy.actor_representation[k].init_hidden(batch)
                for k in self.model_keys
            }
            rnn_hidden_critic = {
                k: self.policy.critic_representation[k].init_hidden(batch)
                for k in self.model_keys
            }
    ...
    def init_hidden_item(
        ...
        """
        assert self.use_rnn is True, (
            "This method cannot be called when self.use_rnn is False."
        )
        ...
        for k in self.model_keys:
            rnn_hidden_critic[k] = self.policy.critic_representation[
                k
            ].init_hidden_item(b_index, *rnn_hidden_critic[k])
        return rnn_hidden_actor, rnn_hidden_critic
    ...
    def action(
        ...
        **kwargs,
        ...
        obs_input, agents_id, avail_actions_input = self._build_inputs(
            obs_dict, avail_actions_dict
        )
        ...
        if self.use_parameter_sharing:
            ...
            actions_dict = [
                {
                    k: actions_out[e, i].cpu().detach().numpy()
            ...
            if not test_mode:
                log_pi_a = (
                    pi_dists[key]
                    .log_prob(actions_sample)
                    .cpu()
                    .detach()
                    .numpy()
                    .reshape(n_env, self.n_agents)
                )
                log_pi_a_dict = {
                    k: log_pi_a[:, i] for i, k in enumerate(self.agent_keys)
                }
                values_out[key] = values_out[key].reshape(n_env, self.n_agents)
                values_dict = {
                    k: values_out[key][:, i].cpu().detach().numpy()
                    for i, k in enumerate(self.agent_keys)
                }
        else:
            actions_sample = {
                k: pi_dists[k].stochastic_sample() for k in self.agent_keys
            }
            if self.continuous_control:
                actions_dict = [
                    {
                        k: actions_sample[k].cpu().detach().numpy()[e].reshape([-1])
                        for k in self.agent_keys
                    }
                    for e in range(n_env)
                ]
                ...
                actions_dict = [
                    {
                        k: actions_sample[k].cpu().detach().numpy()[e].reshape([])
                        for k in self.agent_keys
                    }
                    for e in range(n_env)
                ]
            if not test_mode:
                log_pi_a = {
                    k: pi_dists[k].log_prob(actions_sample[k]).cpu().detach().numpy()
                    for k in self.agent_keys
                }
                log_pi_a_dict = {
                    k: log_pi_a[k].reshape([n_env])
                    for i, k in enumerate(self.agent_keys)
                }
                values_dict = {
                    k: values_out[k].cpu().detach().numpy().reshape([n_env])
                    for k in self.agent_keys
                }
    ...
    def values_next(
        ...
        if self.use_parameter_sharing:
            key = self.agent_keys[0]
            if self.use_rnn:
                hidden_item_index = np.arange(
                    i_env * self.n_agents, (i_env + 1) * self.n_agents
                )
                rnn_hidden_critic_i = {
            ...
            values_dict = {
                k: values_out[i].cpu().detach().numpy()
                for i, k in enumerate(self.agent_keys)
            }
            ...
            obs_input = (
                {k: obs_dict[k][None, :] for k in self.agent_keys}
                if self.use_rnn
                else obs_dict
            )
            ...
            values_dict = {
                k: values_out[k].cpu().detach().numpy().reshape([])
                for k in self.agent_keys
            }
        ...
        if self.memory.full:
            ...
            for _ in range(n_epochs):
                ...
                for start in range(0, self.buffer_size, self.batch_size):
                    ...
                    info_train = (
                        self.learner.update_rnn(sample)
                        if self.use_rnn
                        else self.learner.update(sample)
                    )
            self.memory.clear()
        return info_train
    ...
    def train(self, n_steps):
        ...
        for _ in tqdm(range(n_steps)):
            ...
            next_avail_actions = (
                self.envs.buf_avail_actions if self.use_actions_mask else None
            )
                ...
                for i in range(self.n_envs):
                    if all(terminated_dict[i].values()):
                        ...
                        _, value_next = self.values_next(
                            i_env=i, obs_dict=next_obs_dict[i], state=next_state_i
                        )
            ...
            self.log_infos(train_info, self.current_step)
            return_info.update(train_info)
            obs_dict, avail_actions = (
                deepcopy(next_obs_dict),
                deepcopy(next_avail_actions),
            )
            ...
            for i in range(self.n_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    if all(terminated_dict[i].values()):
                        ...
                        _, value_next = self.values_next(
                            i_env=i, obs_dict=obs_dict[i], state=state_i
                        )
                    self.memory.finish_path(
                        i_env=i,
                        value_next=value_next,
                    ...
                    if self.use_wandb:
                        step_info[
                            f"Train-Results/Episode-Steps/rank_{self.rank}/env-%d" % i
                        ] = info[i]["episode_step"]
                        step_info[
                            f"Train-Results/Episode-Rewards/rank_{self.rank}/env-%d" % i
                        ] = info[i]["episode_score"]
                    else:
    ...
    def run_episodes(self, env_fn=None, n_episodes: int = 1, test_mode: bool = False):
        ...
        while episode_count < n_episodes:
            ...
            rnn_hidden_actor, rnn_hidden_critic = (
                policy_out["rnn_hidden_actor"],
                policy_out["rnn_hidden_critic"],
            )
            actions_dict, log_pi_a_dict = policy_out["actions"], policy_out["log_pi"]
            values_dict = policy_out["values"]
            ...
            next_avail_actions = (
                envs.buf_avail_actions if self.use_actions_mask else None
            )
            if test_mode:
                if self.config.render_mode == "rgb_array" and self.render:
                    images = envs.render(self.config.render_mode)
            ...
            for i in range(num_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    episode_count += 1
                    episode_score = float(
                        np.mean(itemgetter(*self.agent_keys)(info[i]["episode_score"]))
                    )
                    scores.append(episode_score)
                    if test_mode:
                        if self.use_rnn:
                            rnn_hidden_actor, _ = self.init_hidden_item(
                                i, rnn_hidden_actor
                            )
                        ...
                        if self.use_wandb:
                            ...
                            step_info["Train-Results/Episode-Rewards/env-%d" % i] = (
                                info[i]["episode_score"]
                            )
                        else:
        ...
        if test_mode:
            if self.config.render_mode == "rgb_array" and self.render:
                ...
                videos_info = {
                    "Videos_Test": np.array([episode_videos], dtype=np.uint8).transpose(
                        (0, 1, 4, 2, 3)
                    )
                }
                self.log_videos(
                    info=videos_info, fps=self.fps, x_index=self.current_step
                )
