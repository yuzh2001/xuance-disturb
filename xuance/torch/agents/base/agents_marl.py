...
class MARLAgents(ABC):
    ...
    def __init__(
        ...
        self.use_actions_mask = (
            config.use_actions_mask if hasattr(config, "use_actions_mask") else False
        )
        self.use_global_state = (
            config.use_global_state if hasattr(config, "use_global_state") else False
        )
        ...
        self.start_training = (
            config.start_training if hasattr(config, "start_training") else 1
        )
        self.training_frequency = (
            config.training_frequency if hasattr(config, "training_frequency") else 1
        )
        ...
        self.episode_length = (
            ...
            if hasattr(config, "episode_length")
            else envs.max_episode_steps
        )
        ...
        if self.distributed_training and self.world_size > 1:
            if self.rank == 0:
                ...
                time_string_tensor = torch.tensor(
                    list(time_string.encode("utf-8")), dtype=torch.uint8
                ).to(self.rank)
            else:
            ...
            time_string = (
                bytes(time_string_tensor.cpu().tolist()).decode("utf-8").rstrip("\x00")
            )
        else:
            time_string = get_time_string()
        seed = f"seed_{config.seed}_"
        self.model_dir_load = config.model_dir
        self.model_dir_save = os.path.join(
        ...
        for key in self.model_keys:
            if self.use_rnn:
                ...
                hidden_sizes = (
                    ...
                    if hasattr(config, "representation_hidden_size")
                    else None
                )
            input_representations = dict(
                input_shape=space2shape(input_space[key]),
                hidden_sizes=hidden_sizes,
                normalize=NormalizeFunctions[config.normalize]
                ...
                fc_hidden_sizes=config.fc_hidden_sizes
                if hasattr(config, "fc_hidden_sizes")
                else None,
                N_recurrent_layers=config.N_recurrent_layers
                if hasattr(config, "N_recurrent_layers")
                else None,
                rnn=config.rnn if hasattr(config, "rnn") else None,
                dropout=config.dropout if hasattr(config, "dropout") else None,
                device=self.device,
            )
            representation[key] = REGISTRY_Representation[representation_key](
                **input_representations
            )
            if representation_key not in REGISTRY_Representation:
                raise AttributeError(
                    f"{representation_key} is not registered in REGISTRY_Representation."
                )
    ...
    def _build_inputs(
        self, obs_dict: List[dict], avail_actions_dict: Optional[List[dict]] = None
    ):
        """
        ...
        if self.use_parameter_sharing:
            ...
            obs_array = np.array(
                [itemgetter(*self.agent_keys)(data) for data in obs_dict]
            )
            agents_id = (
            ...
            avail_actions_array = (
                np.array(
                    [itemgetter(*self.agent_keys)(data) for data in avail_actions_dict]
                )
            ...
            if self.use_rnn:
                ...
                if self.use_actions_mask:
                    avail_actions_input = {
                        key: avail_actions_array.reshape([bs, 1, -1])
                    }
            else:
            ...
            if self.use_rnn:
                obs_input = {
                    k: np.stack([data[k] for data in obs_dict]).reshape([bs, 1, -1])
                    for k in self.agent_keys
                }
                if self.use_actions_mask:
                    avail_actions_input = {
                        k: np.stack([data[k] for data in avail_actions_dict]).reshape(
                            [bs, 1, -1]
                        )
                        for k in self.agent_keys
                    }
                ...
                obs_input = {
                    k: np.stack([data[k] for data in obs_dict]).reshape(bs, -1)
                    for k in self.agent_keys
                }
                if self.use_actions_mask:
                    avail_actions_input = {
                        k: np.array([data[k] for data in avail_actions_dict]).reshape(
                            [bs, -1]
                        )
                        for k in self.agent_keys
                    }
        ...
        if self.distributed_training:
            if dist.get_rank() == 0:
                if os.path.exists(self.learner.snapshot_path):
                    if os.path.exists(
                        os.path.join(self.learner.snapshot_path, "snapshot.pt")
                    ):
                        os.remove(
                            os.path.join(self.learner.snapshot_path, "snapshot.pt")
                        )
...
class RandomAgents(object):
    ...
    def action(self, obs_n, episode, test_mode, noise=False):
        rand_a = [
            [self.action_space[agent].sample() for agent in self.agent_keys]
            for e in range(self.nenvs)
        ]
