...
class MAPPO_Agents(IPPO_Agents):
    ...
    def _build_policy(self) -> Module:
        ...
        activation_action = (
            ActivationFunctions[self.config.activation_action]
        ...
        if self.use_global_state:
            ...
            dim_obs_all = sum(
                [sum(self.observation_space[k].shape) for k in self.agent_keys]
            )
        ...
        if self.config.policy in ["Categorical_MAAC_Policy", "Gaussian_MAAC_Policy"]:
            ...
            self.continuous_control = (
                True if "Gaussian" in self.config.policy else False
            )
        else:
            raise AttributeError(
                f"MAPPO currently does not support the policy named {self.config.policy}."
            )
        return policy

    def _build_critic_inputs(
        self, batch_size: int, obs_batch: dict, state: Optional[np.ndarray]
    ):
        """
        ...
        if self.use_global_state:
            if self.use_parameter_sharing:
                ...
                state_n = np.stack(
                    [state for _ in range(self.n_agents)], axis=1
                ).reshape([bs, -1])
            ...
            joint_obs = (
                joint_obs.reshape([bs, 1, -1])
                if self.use_rnn
                else joint_obs.reshape([bs, -1])
            )
    ...
    def action(
        ...
        **kwargs,
        ...
        if not test_mode:
            critic_input = self._build_critic_inputs(
                batch_size=n_env, obs_batch=obs_input, state=state
            )
    ...
    def values_next(
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                if self.use_global_state:
                    critic_input = np.repeat(
                        state.reshape([n_env, 1, -1]), self.n_agents, axis=1
                    ).reshape([batch_size, 1, -1])
                else:
                    obs_array = np.array(itemgetter(*self.agent_keys)(obs_dict))
                    critic_input = np.repeat(
                        obs_array.reshape([n_env, 1, -1]), self.n_agents, axis=1
                    ).reshape([batch_size, 1, -1])
                agents_id = (
                ...
                if self.use_global_state:
                    ...
                    obs_array = np.array(
                        [itemgetter(*self.agent_keys)(obs_dict)]
                    ).reshape([n_env, 1, -1])
            ...
            if self.use_rnn:
                ...
                joint_obs = np.stack(
                    itemgetter(*self.agent_keys)(obs_dict), axis=0
                ).reshape([n_env, 1, -1])
