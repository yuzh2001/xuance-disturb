...
class MAPPO_Clip_Learner(IPPO_Learner):
    ...
    def update(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_global_state:
                critic_input = {
                    key: state.reshape(batch_size, 1, -1)
            ...
            if self.use_global_state:
                critic_input = {
                    k: state.reshape(batch_size, -1) for k in self.agent_keys
                }
            else:
        ...
        # feedforward
        _, pi_dists_dict = self.policy(
            observation=obs, agent_ids=IDs, avail_actions=avail_actions
        )
        _, value_pred_dict = self.policy.get_values(
            observation=critic_input, agent_ids=IDs
        )
        ...
        for key in self.model_keys:
            ...
            surrogate2 = (
                torch.clip(ratio, 1 - self.clip_range, 1 + self.clip_range)
                * advantages_mask
            )
            ...
            # 计算近似KL散度
            with torch.no_grad():
                log_ratio = log_pi - log_pi_old[key]
                approx_kl_div = ((torch.exp(log_ratio) - 1) - log_ratio).mean()
            ...
            if self.use_value_clip:
                ...
                if self.use_value_norm:
                    ...
                    value_target = (
                        self.value_normalizer[key]
                        .normalize(value_target.reshape(bs, 1))
                        .reshape(bs)
                    )
            ...
            info.update(
                {
                    ...
                    f"{key}/approx_kl": approx_kl_div.item(),
                }
            )
        ...
        if self.use_grad_clip:
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.policy.parameters_model, self.grad_clip_norm
            )
    ...
    def update_rnn(self, sample):
        ...
        state = sample_Tensor["state"]
        bs_rnn = (
            batch_size * self.n_agents if self.use_parameter_sharing else batch_size
        )
        obs = sample_Tensor["obs"]
        actions = sample_Tensor["actions"]
        values = sample_Tensor["values"]
        ...
        if self.use_parameter_sharing:
            ...
            filled = (
                filled.unsqueeze(1)
                .expand(batch_size, self.n_agents, seq_len)
                .reshape(bs_rnn, seq_len)
            )
            if self.use_global_state:
                critic_input = {
            ...
            if self.use_global_state:
                critic_input = {
                    k: state.reshape(batch_size, seq_len, -1) for k in self.agent_keys
                }
            else:
        ...
        rnn_hidden_actor = {
            k: self.policy.actor_representation[k].init_hidden(bs_rnn)
            for k in self.model_keys
        }
        rnn_hidden_critic = {
            k: self.policy.critic_representation[k].init_hidden(bs_rnn)
            for k in self.model_keys
        }

        # feedforward
        _, pi_dist_dict = self.policy(
            obs, agent_ids=IDs, avail_actions=avail_actions, rnn_hidden=rnn_hidden_actor
        )
        _, value_pred_dict = self.policy.get_values(
            critic_input, agent_ids=IDs, rnn_hidden=rnn_hidden_critic
        )
        ...
        for key in self.model_keys:
            ...
            loss_a.append(
                -(torch.min(surrogate1, surrogate2) * mask_values).sum()
                / mask_values.sum()
            )
            ...
            if self.use_value_clip:
                ...
                if self.use_value_norm:
                    ...
                    value_target = self.value_normalizer[key].normalize(
                        value_target.reshape(-1, 1)
                    )
